 ...
 * comments, ignore blank lines. Deal with quoted (\) #, and quoted
...
#if MSDOS || OS2
	if (cont) {
	    /* delete all chars from p to first non-space */
	    char *a;
	    q = a = str + pos;
	    while (isspace(*a))
		a++;
...
/*
 * Get a word from the current line, surounded by white space. return
 * a pointer to it. String returned has no white spaces in it.
 */
char *
gettok(ptr)
char **ptr;
