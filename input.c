...
/*
 * Add a command to the end of the supplied list of commands. Return
 * the new head pointer for that list.
 */
struct cmd *
newcmd(str, cp)
char *str;
...
/*
 * Add a new 'line' of stuff to a target.  This check to see if
 * commands already exist for the target.  If flag is set, the line
 * is a double colon target.
 *
 * Kludges: i)	If the new name begins with a '.', and there are no
 * dependents, then the target must cease to be a target.  This is
 * for .SUFFIXES. ii) If the new name begins with a '.', with no
 * dependents and has commands, then replace the current commands.
 * This is for redefining commands for a default rule. Neither of
 * these free the space used by dependents or commands, since they
 * could be used by another target.
 */
void
...
/*
 * Parse input from the makefile, and construct a tree structure of
 * it.
 */
void
input( FILE *fd)
{
	...
	while (((q = index(p, ':')) != (char *) 0) &&
	    (p != q) && (q[-1] == contchar)) {	    /* Find dependents	*/
	    register char *a;
